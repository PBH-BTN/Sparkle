...
public class HTTPUtil {
    ...
    public static void onProgress(ProgressTracker.Progress progress) {
        if (progress.determinate()) { // Overall progress can be measured
            ...
            log.info("Downloading: {}/{} ({})", progress.totalBytesTransferred(), progress.contentLength(), String.format("%.2f", percent));
        } else {
            log.info("Downloading: {}/{}", progress.totalBytesTransferred(), progress.contentLength());
        }
        if (progress.done()) {
            log.info("Finished: {}/{}", progress.totalBytesTransferred(), progress.contentLength());
        }
    }
