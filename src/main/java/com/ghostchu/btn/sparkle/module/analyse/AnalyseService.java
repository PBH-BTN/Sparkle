...
SELECT time_bucket('7 day', "insert_time") AS bucket, peer_ip, COUNT(DISTINCT user_application) FROM banhistory
        ...
        filtered.nodeIterator(false).forEachRemaining(node -> rules.add(new AnalysedRule(null, node.getKey().toString(), UNTRUSTED_IP,
    ...
    public void cronHighRiskIps() {
        ...
        var filtered = filterIP(ipTries);
        List<AnalysedRule> rules = new ArrayList<>();
        filtered.nodeIterator(false).forEachRemaining(node -> rules.add(new AnalysedRule(null, node.getKey().toString(), HIGH_RISK_IP, "Auto Generated by Sparkle")));
        analysedRuleRepository.replaceAll(HIGH_RISK_IP, rules);
        meterRegistry.gauge("sparkle_analyse_high_risk_ips", Collections.emptyList(), rules.size());
        log.info("High risk IPs: {}, tooked {} ms", rules.size(), System.currentTimeMillis() - startAt);
    }
...
//    }
    ...
    public void cronUpdateTrunkerFile() {
        ...
        while (it.hasNext()) {
            ...
            rules.add(new AnalysedRule(null, node.getKey().toString(), TRACKER_HIGH_RISK, ByteUtil.filterUTF8("[AutoGen] " + info)));
        }
